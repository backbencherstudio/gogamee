...
const calculatePriceFromDefault = (
  timeRange: TimeRange,
  isDeparture: boolean,
): number => {
...
const TimeDisplay = React.memo(
  ({
    ...
    const startTime = useMemo(
      () => minutesToTime(timeRange.start),
      [timeRange.start],
    );
    const endTime = useMemo(
      () => minutesToTime(timeRange.end),
      [timeRange.end],
    );
...
const FlightCard = React.memo(
  ({
    ...
    const currentPrice = calculatePriceFromDefault(
      flightInfo.timeRange,
      isDeparture,
    );
    const priceDisplay = currentPrice === 0 ? "0€" : `+${currentPrice}€`;

    return (
      <div className="flex-1 min-h-0 p-4 xl:p-5 bg-white rounded-lg ring-1 ring-inset ring-gray-200 flex flex-col justify-start items-center gap-6 xl:gap-8">
...
export default function FlightSchedule() {
  ...
  const handleTimeRangeChange = useCallback(
    (flightIndex: number, newRange: TimeRange) => {
      ...
      if (startSlotIndex === endSlotIndex) {
        if (endSlotIndex < timeSlots.length - 1) {
          ...
          adjustedRange = {
            ...adjustedRange,
            end: timeSlots[endSlotIndex + 1].value,
          };
        } else if (startSlotIndex > 0) {
          // Move start time one step backward
          adjustedRange = {
            ...adjustedRange,
            start: timeSlots[startSlotIndex - 1].value,
          };
        }
      }
  ...
  const totalAdditionalCost = useMemo(() => {
    const departureCost = calculatePriceFromDefault(
      flightData[0].timeRange,
      true,
    );
    const arrivalCost = calculatePriceFromDefault(
      flightData[1].timeRange,
      false,
    );
            ...
            {flightData.map((flight, index) => (
              <FlightCard
                ...
                onTimeRangeChange={(range) =>
                  handleTimeRangeChange(index, range)
                }
              />
            ))}
          </div>
