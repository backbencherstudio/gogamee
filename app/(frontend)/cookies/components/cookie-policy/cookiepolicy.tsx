...
export default function CookiePolicy() {
  ...
  const loadContent = async () => {
    try {
      setLoading(true)
      setError(null)
      const response = await getLegalPageContent('cookie', language === 'es' ? 'es' : 'en')
      ...
      <div className='w-full max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8'>
        <div className="w-full flex flex-col justify-start items-center gap-8 lg:gap-12 py-12 sm:py-16 lg:py-[100px]">
          <div className="flex justify-center items-center py-12">
            <div className="text-neutral-600 text-lg font-medium">Loading cookie policy...</div>
          </div>
        </div>
      </div>
    )
  ...
  if (!content) {
    return (
      <div className='w-full max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8'>
        <div className="w-full flex flex-col justify-start items-center gap-8 lg:gap-12 py-12 sm:py-16 lg:py-[100px]">
          <div className="flex justify-center items-center py-12">
            <div className="text-neutral-600 text-lg font-medium">Cookie policy content is not available.</div>
          </div>
        </div>
      </div>
    )
