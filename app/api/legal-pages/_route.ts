  ...
  try {
    ...
    pages.forEach((p) => {
      ...
      if (["privacy", "cookie", "terms"].includes(p.type)) {
        // This part depends on how ILegalPage handles languages.
        // If generic content string, maybe we return that.
        // If the user expects { en, es }, we need to check if schema supports it.
        // Let's assume simple content for now or check schema if needed.
        // Re-reading user code: "content[page].en = page.content" in previously seen route.
        // So it seems pages are single language or we just put content in 'en' slot?
        // User code had: content[page].en = page.content.
        // Let's just return what we have.

        const key = p.type as "privacy" | "cookie" | "terms";
        // Simple mapping if we don't have language specific rows
        content[key].es = p.content;
        content[key].en = p.content;
      }
    });
